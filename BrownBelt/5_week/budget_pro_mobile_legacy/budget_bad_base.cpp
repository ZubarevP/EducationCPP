...
array<double, VERTEX_COUNT> tree_add, tree_factor, tree_spend;
...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        ...
        (tree_values[w].earn *= tree_factor[v]) += tree_add[v] * (r - l) / 2;

        tree_spend[w] += tree_spend[v];
        tree_values[w].spend += tree_spend[v] * (r - l) / 2;
        }
    }
    tree_factor[v] = 1;
    tree_add[v] = 0;
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
    ...
    Push(v, l, r);
    if (ql <= l && r <= qr) {
      return tree_values[v].earn - tree_values[v].spend;
    }
    return ComputeSum(v * 2, l, (l + r) / 2, ql, qr)
        + ComputeSum(v * 2 + 1, (l + r) / 2, r, ql, qr);
}

void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, Money money) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
      return;
    }
    Push(v, l, r);
    if (ql <= l && r <= qr) {
      tree_add[v] += money.earn;
      tree_values[v].earn += money.earn * (r - l);
      tree_spend[v] += money.spend;
      tree_values[v].spend += money.spend * (r - l);
      return;
    }

    Add(v * 2, l, (l + r) / 2, ql, qr, money);
    Add(v * 2 + 1, (l + r) / 2, r, ql, qr, money);
    tree_values[v].earn =
        (v * 2 < VERTEX_COUNT ? tree_values[v * 2].earn : 0)
        + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1].earn : 0);
    tree_values[v].spend = 
        (v * 2 < VERTEX_COUNT ? tree_values[v * 2].spend : 0)
        + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1].spend : 0);
}
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            ...
            cin >> value;
            Money money;
            money.earn = value / (idx_to - idx_from);
            money.spend = 0.;
            Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, money);

        } else if (query_type == "Spend") {
            double value;
            cin >> value;
            Money money;
            money.spend = value / (idx_to - idx_from);
            money.earn = 0.;
            Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, money);
        }
    }
