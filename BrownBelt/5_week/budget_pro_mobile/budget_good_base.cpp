...
public:
    ...
    BulkLinearUpdater(const BulkSpendAdder& spend)
        : spend_ (spend)
    {}
    ...
    void CombineWith(const BulkLinearUpdater& other) {
        ...
        spend_.value += other.spend_.value;
    }

    double CollapseAdd(double origin, IndexSegment segment) const {
        return  origin * tax_.factor + 
                add_.delta * segment.length();
    }

    double CollapseSpend(double origin, IndexSegment segment) const {
        return origin  + spend_.value * segment.length();
    }
...
public:
    ...
    Data ComputeSum(IndexSegment segment) const {
        auto result = this->TraverseWithQuery(root_, segment, ComputeSumVisitor{});
        return result.data - result.expenses;
    }
...
private:
  ...
  public:
    using ResultType = Money;

    Money ProcessEmpty(const NodeHolder&) const {
      return {};
    }

    Money ProcessFull(const NodeHolder& node) const {
      return node->money;
    }

    Money ProcessPartial(const NodeHolder&, IndexSegment, const Money& left_result, const Money& right_result) const {
      return {left_result.data + right_result.data, left_result.expenses + right_result.expenses};
    }
  };
    ...
    public:
        ...
        // node == root_, operation == visitor(money add, tax, money spend)
        void ProcessFull(const NodeHolder& node) const {
            // добавляем в рот из полученного запроса новае данные 
            node->postponed_bulk_operation.CombineWith(operation_);
            // 
            node->money.data = operation_.CollapseAdd(node->money.data, node->segment);
            node->money.expenses = operation_.CollapseSpend(node->money.expenses, node->segment);
        }

        void ProcessPartial(const NodeHolder& node, IndexSegment) const {
            node->money.data =     (node->left ? node->left->money.data : Data()) + 
                                   (node->right ? node->right->money.data : Data());
            node->money.expenses = (node->left ? node->left->money.expenses : Data()) +
                                   (node->right ? node->right->money.expenses : Data()); 

        }

    private:

        // struct: tax, money add, money spend;
    ...
    static void PropagateBulkOperation(const NodeHolder& node) {
        ...
        if (child_ptr) {
            ...
            child_ptr->money.data = node->postponed_bulk_operation.CollapseAdd(
                child_ptr->money.data, child_ptr->segment
            );

            child_ptr->money.expenses = node->postponed_bulk_operation.CollapseSpend(
                child_ptr->money.expenses, child_ptr->segment
            );
        }
        }
...
struct SpendRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        manager.AddBulkOperation(date_segment, BulkSpendAdder{daily_spend});
    }
