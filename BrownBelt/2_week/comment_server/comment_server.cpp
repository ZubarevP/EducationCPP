...
public:
  ...
  HttpResponse& AddHeader(string name, string value) {
      head.push_back({move(name), move(value)});
      return *this;
    }
  ...
  friend ostream& operator << (ostream& output, const HttpResponse& resp) {
    output << "HTTP/1.1 ";
    switch(resp.code) {
        case HttpCode::Ok : output << "200 OK\n"; break;
        case HttpCode::Found : output << "302 Found\n"; break;
        case HttpCode::NotFound : output << "404 Not found\n"; break;
    }
    for(const auto& line :  resp.head) {
  ...
  HttpResponse ServeRequest(const HttpRequest& req) {
    if (req.method == "POST") {
      ...
      if (req.path == "/user_comments") {
        ...
        string line("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
        return  HttpResponse(HttpCode::Ok).SetContent(line);
...
"HTTP/1.1 200 OK\nContent-Length: 1\n\n0"
 CommentServer: incorrect response for POST /add_user 
*/